...
This copy parses `TF2.md` alone, this tree carrying no second ledger:
one entry per Python file of Core's `test/functional/test_framework/`,
each pinned to a commit under a heading owning one fenced block, the
...
What this script does not do is notice a file Core *gains*: it walks
only the entries a ledger already carries, so a new
`test_framework/*.py` upstream is invisible to it.
`tf2_ledger_census.py` is the separate, weekly job that reads the
directory itself and reports that question under a title of its own.
...
No current entry uses the placeholder shape. Every heading the ledger
...
all.
    ...
    python .github/scripts/check_vendored_vectors.py \
        TF2.md "TF2.md pins behind upstream"
"""
