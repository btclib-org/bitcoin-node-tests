...
- Does the diff **import a node**, in-process, rather than reach one
  over a process, an RPC socket or a p2p socket? Rule 1 of
...
- Does a change to `TF2.md` or to `.github/scripts/tf2_ledger_census.py`
  **reintroduce the `contents` endpoint** for a directory listing?
  `TF2.md`'s own *Re-checking a pin* section says why that endpoint is
